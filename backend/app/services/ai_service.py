...
class AIService:
    ...
    async def query_project(self, project_id: str, query_text: str) -> Tuple[str, List[NodeWithScore]]:
        ...
        # --- ADDED: Load explicit Plan and Synopsis ---
        ...
        try:
            ...
            try:
             ...
             logger.error(f"AIService (Query): Error loading explicit context for project {project_id}: {e}", exc_info=True)
             # Continue without explicit context, but log the error
             explicit_plan = "Error loading plan."
             explicit_synopsis = "Error loading synopsis."
        # --- END ADDED ---

        # --- MODIFIED: Pass explicit context to rag_engine.query ---
    ...
    async def generate_scene_draft(self, project_id: str, chapter_id: str, request_data: AISceneGenerationRequest) -> str:
        ...
        try:
            # Load Explicit Context using FileService (Plan, Synopsis)
